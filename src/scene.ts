...
function softShadow(){
 const canvas=document.createElement('canvas');canvas.width=canvas.height=128;const ctx=canvas.getContext('2d')!,gradient=ctx.createRadialGradient(64,64,3,64,64,62);
 gradient.addColorStop(0,'rgba(255,255,255,.72)');gradient.addColorStop(.42,'rgba(255,255,255,.4)');gradient.addColorStop(.76,'rgba(255,255,255,.12)');gradient.addColorStop(1,'rgba(255,255,255,0)');ctx.fillStyle=gradient;ctx.fillRect(0,0,128,128);
 const texture=new T.CanvasTexture(canvas);texture.colorSpace=T.SRGBColorSpace;return texture;
}
export class Arena {
 renderer:T.WebGLRenderer; scenes:T.Scene[]=[]; cameras:T.PerspectiveCamera[]=[]; flies:T.Group[]=[]; rocks:T.Mesh[]=[]; rings:T.Mesh[]=[]; collisionAt:(number|null)[]=[null,null];hosts:HTMLElement[];canvas:HTMLCanvasElement;
 constructor(host:HTMLElement,lanes:HTMLElement[]){this.hosts=lanes;this.renderer=new T.WebGLRenderer({antialias:true,alpha:true});this.renderer.setPixelRatio(Math.min(devicePixelRatio,2));this.renderer.setClearColor(0x090d14,0);this.canvas=this.renderer.domElement;host.prepend(this.canvas);this.canvas.className='arena-canvas';this.renderer.setScissorTest(true);const shadowMap=softShadow();
 ...
 const ring=new T.Mesh(new T.PlaneGeometry(.024,.011),new T.MeshBasicMaterial({map:shadowMap,color:0x7893a5,transparent:true,opacity:.22,depthWrite:false,side:T.DoubleSide}));ring.rotation.x=-Math.PI/2;ring.position.y=-.0095;s.add(ring);
 ...
 for(let i=0;i<2;i++){const r=this.hosts[i].getBoundingClientRect(),x=r.left-rect.left,y=rect.bottom-r.bottom;this.renderer.setViewport(x,y,r.width,r.height);this.renderer.setScissor(x,y,r.width,r.height);const cam=this.cameras[i];cam.aspect=r.width/r.height;cam.updateProjectionMatrix();const f=this.flies[i],dodge=dodgeX(trial,t,actions[i]),crashed=collisions[i]&&t>540;if(crashed&&this.collisionAt[i]===null)this.collisionAt[i]=now;if(!crashed)this.collisionAt[i]=null;const impact=crashed?clamp((now-this.collisionAt[i]!)/320):0;f.position.x=dodge*(1-recenter);f.position.y=-.006*impact;f.rotation.x=.9*impact;f.rotation.z=-dodge*5+(i?1:-1)*1.15*impact;f.visible=true;for(const wing of f.children.filter(c=>c.name==='wing'))wing.rotation.z=Math.sin(now*.07)*.13*(1-impact);
 const rock=this.rocks[i];rock.visible=visible&&t>=0;const p=trial.object.position,v=trial.object.velocity;rock.position.set(p[0]+v[0]*t/1000,p[1]+v[1]*t/1000,p[2]+v[2]*t/1000);rock.rotation.set(t*.0004,t*.0007,.3);const ring=this.rings[i],ringMaterial=ring.material as T.MeshBasicMaterial,pulse=crashed?Math.sin(impact*Math.PI):0;ring.position.x=f.position.x;ring.scale.setScalar(1+pulse*.7);ringMaterial.color.setHex(crashed?0xff6b70:0x7893a5);ringMaterial.opacity=crashed?.62+pulse*.28:.22;this.renderer.render(this.scenes[i],cam);}
