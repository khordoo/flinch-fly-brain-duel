...
export class Arena {
 ...
 const grid=new T.GridHelper(1,40,0x233244,0x142130);grid.position.set(0,-.01,.35);s.add(grid);
 for(let x of [-.065,.065]){const line=new T.BufferGeometry().setFromPoints([new T.Vector3(x,-.009,-.03),new T.Vector3(x,-.009,.8)]);s.add(new T.Line(line,new T.LineBasicMaterial({color:0x3a5267,transparent:true,opacity:.6})));}
 for(let z=.12;z<.9;z+=.14){const points=[new T.Vector3(-.085,-.01,z),new T.Vector3(-.085,.07,z),new T.Vector3(.085,.07,z),new T.Vector3(.085,-.01,z)];s.add(new T.Line(new T.BufferGeometry().setFromPoints(points),new T.LineBasicMaterial({color:0x233442,transparent:true,opacity:.48})));}
 ...
 for(let i=0;i<2;i++){const r=this.hosts[i].getBoundingClientRect(),x=r.left-rect.left,y=rect.bottom-r.bottom;this.renderer.setViewport(x,y,r.width,r.height);this.renderer.setScissor(x,y,r.width,r.height);const cam=this.cameras[i];cam.aspect=r.width/r.height;cam.updateProjectionMatrix();const f=this.flies[i];f.position.x=dodgeX(trial,t,actions[i]);f.rotation.z=-dodgeX(trial,t,actions[i])*5;f.position.y=0;f.visible=!(collisions[i]&&t>540);for(const wing of f.children.filter(c=>c.name==='wing'))wing.rotation.z=Math.sin(now*.07)*.13;
