...
export class Arena {
 ...
 const grid=new T.GridHelper(1,40,0x31506a,0x1c3042);grid.position.set(0,-.01,.35);s.add(grid);
 for(let x of [-.065,.065]){const line=new T.BufferGeometry().setFromPoints([new T.Vector3(x,-.009,-.03),new T.Vector3(x,-.009,.8)]);s.add(new T.Line(line,new T.LineBasicMaterial({color:0x557590,transparent:true,opacity:.68})));}
 for(let z=.12;z<.9;z+=.14){const points=[new T.Vector3(-.085,-.01,z),new T.Vector3(-.085,.07,z),new T.Vector3(.085,.07,z),new T.Vector3(.085,-.01,z)];s.add(new T.Line(new T.BufferGeometry().setFromPoints(points),new T.LineBasicMaterial({color:0x34516a,transparent:true,opacity:.56})));}
